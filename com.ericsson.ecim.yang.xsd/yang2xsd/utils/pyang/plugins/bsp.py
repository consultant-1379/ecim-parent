"""XSD output plugin"""
#this is based on the "xsd" plugin for pyang (deprecated by the way) and
#modified for bsp8100, to include actions, and add more info about augments
#also a new argument --bsp-linkxsd is added to permit running a second pass
#to link the generated .xsd fragments together according to the included
#information about augments.
...
class XSDPlugin(plugin.PyangPlugin):
    def add_opts(self, optparser):
        optlist = [
            optparse.make_option("--bsp-global-complex-types",
                                 dest="bsp_global_complex_types",
                                 action="store_true",
                                 help="Make all complex types global instead" +
                                      " of inline"),
            optparse.make_option("--bsp-groups",
                                 dest="bsp_groups",
                                 action="store_true",
                                 help="EXPERIMENTAL: does not work correctly"),
            optparse.make_option("--bsp-no-appinfo",
                                 dest="bsp_no_appinfo",
                                 action="store_true",
                                 help="Do not print YANG specific appinfo"),
            optparse.make_option("--bsp-no-imports",
                                 dest="bsp_no_imports",
                                 action="store_true",
                                 help="Do not generate xs:import elements"),
            optparse.make_option("--bsp-break-pattern",
                                 dest="bsp_break_pattern",
                                 action="store_true",
                                 help="Break bsp pattern so that they fit "
                                      "into RFCs"),
            optparse.make_option("--bsp-no-lecture",
                                 dest="bsp_no_lecture",
                                 action="store_true",
                                 help="Do not generate the lecture about "
                                      "how the XSD can be used"),
            optparse.make_option("--bsp-linkxsd",
                                 dest="bsp_linkxsd",
                                 help="Link together all xsd documents found in indicated directory...")
            ]
        g = optparser.add_option_group(
            "Ericsson BSP xml output specific options")
        ...
        for pre in module.i_prefixes:
            ...
            if mod == None:
                raise error.EmitError("cannot find module %s, needed by XSD"
                                      " translation" % modname)
#normal run if bsp_linkxsd is undefined, otherwise it is a second
#pass to add imports and references due to augments
        if ctx.opts.bsp_linkxsd == None:   
            emit_xsd(ctx, module, fd)
        else:
            fix_augments(ctx, module, fd)
...
#Open the .xsd document corresponding to the .yang being handled
#(assumed to have been created during an earlier run)
#Look for "AUGMENT" comments in the document.
#call "do_fix_augment" for each such comment, to add the corresponding
#declarations to the document which is being augmented.
def fix_augments(ctx, module, fd):
    print >> sys.stdout, module.arg
    fdir = ctx.opts.bsp_linkxsd + "/"
    filename = fdir + module.arg + ".xsd"
    try:
                fd = open(filename)
                text = fd.readlines()
                ...
                sys.exit(1)
    for line in text:
       if line.find("<!-- AUGMENT") >= 0:
          do_fix_augment(ctx,fdir, line)
    fd.close
               

#Extract information about the augment from line which
#contains the AUGMENT comment and generate the lines which
#may have to be added in the document which is being augmented
#Add the declarations for the document which contains the
#augment (unless they have been added already)
#then look for the augmented element and add a "group ref"
#to the augment in the "sequence" which is assumed to follow.

def do_fix_augment(ctx, fdir, line):
      print >> sys.stdout, line
      elements = line.strip().split(" ")
      ref = elements[2]
      augmx = elements[4].split("/")
      augmy = augmx[-1].split(":")       
      augin = augmy[0]
      ename = augmy[1] 
      urn = "urn:com:ericsson:ecim"
      urn1 = '"' + urn + ':' + ref +'"'
      name = elements[3]
      l1 = "xmlns:" + ref + '=' + urn1 + '\n'
      l2 = "<xs:import namespace=" + urn1 + '\n'
      l3 = 'schemaLocation="' + ref +'.xsd"/>\n'
      l4 = '<xs:group ref="' + ref + ':' + name +'"/>\n'
      filename = fdir + augin + '.xsd'
      try:
                fd = open(filename)
                docx = fd.readlines()
      except IOError as ex:
      ...
      if fnd == 0:
         ...
         for line in docx:
            ix += 1
            if line.find("xmlns:") >= 0: 
               lastxmlns=ix
         docx.insert(lastxmlns,l3)
         docx.insert(lastxmlns,l2)
      if  ctx.opts.bsp_global_complex_types:
         searchfor = '<xs:complexType name="' + ename + '"'
      else:     
         searchfor = '<xs:element name="' + ename + '"'
      ix=0
      state = 0
      for line in docx:
         ix += 1
...
def emit_xsd(ctx, module, fd):
    ...
    if ctx.opts.bsp_no_appinfo != True:
        fd.write('           ' \
    ...
    if ctx.opts.bsp_no_imports != True:
    ...
    if ctx.opts.bsp_no_lecture != True:
...
#        fd.write('\n  <!-- AUGMENT %s -->\n\n' % c.arg)
    ...
    # print groupings
    if ctx.opts.bsp_groups and len(module.i_groupings) > 0:
        fd.write('\n  <!-- YANG groupings -->\n')
...
#        print cn
        if (type(cn) == tuple):
            cn2 = cn[-1]
            cn = cn2
        if cn in ['action', 'container', 'list', 'leaf', 'leaf-list', 'anyxml',
                  'notification', 'rpc']:
            mino = ""
            maxo = ""
            atype = ""
            ...
            if cn in ['leaf', 'leaf-list']:
                ltype = c.search_one('type')
                if ltype.i_is_derived == False:
                    if ltype.arg == 'empty':
                        atype = ''
                    else:
                        atype = '  type="%s"' % xsd_type_name(ctx, ltype, c)
...
#This has been changed a lot as compared to the original "xsd" implementation,
#mainly to separate the code which is executed for the cases of option
#--bsp-global-complex-types being True and False.
#Note that c.xxxx is a nonstandard feature, added in the statement handling part
#of pyang.
#FIXIT While setting --bsp-global-complex-types to True produces plausible-looking
#.xsd, for some reason this will not compile as we want it in xmlBeans, so it is 
#likely that this part will have to be reworked somewhat.
            if  ctx.opts.bsp_global_complex_types:
                     if cn in ['container']:
                            ctype_name=string.split(c.xxxx, ":")[1]
                     elif cn in ['action', 'list', 'rpc', 'notification']: 
                        ctype_name = c.arg
                        x = c.parent
                        idx = 0
                     ...
                     if cn in ['leaf', 'leaf-list']:
                        ...
                        fd.write(indent + '    </xs:complexContent>\n')
                        fd.write(indent + '  </xs:complexType>\n')
                     if cn in ['action', 'container', 'list', 'rpc', 'notification']: 
                        ustr = ""
...
#
                     ...
                     if cn in ['action', 'list', 'rpc', 'notification']:
                        if not has_body:
                            fd.write('>\n')
                     ...
                     if cn in ['container']:
                                ...
                                if not has_body:
                                        ...
                                        fd.write(indent + '</xs:element>\n')
                                if ctype_name not in ctx.container_name_list: 
                                        ctx.container_name_list.append(ctype_name)                                                                 
                                        # add to queue
                                        ctx.xsd_ct_queue.insert(0, (path, uindent, extbase, cn, c,
                                                   ' name="%s"' % ctype_name))
                     elif inline_end:
...
def print_augment(ctx, module, fd, indent, augment):
    ...
    name = "augment" + str(i)
    #add a comment to indicate what is being augmented.
    #this is used in the second pass to insert references
    #in the documents which are being augmented.
    fd.write('\n  <!-- AUGMENT %s %s %s -->\n\n' % (module.arg, name, augment.arg))
...
#                        '        <xs:length value="%s"/>\n' % lo)
        ...
        def print_pattern(indent, patstr):
            ...
            if ctx.opts.bsp_break_pattern == True and len(qstr) > cnt:
                while (len(qstr) > 0):
                    fd.write(qstr[0:cnt])
                    qstr = qstr[cnt:]
                    if len(qstr):
...
def print_annotation(ctx, fd, indent, obj, prestr=''):
    ...
    if ((ctx.opts.bsp_no_appinfo == False and len(stmts) > 0) or
        ...
        if obj.search_one('description') != None:
            ...
            fd.write(indent + '  </xs:documentation>\n')
        if ctx.opts.bsp_no_appinfo == False:
